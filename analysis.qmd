...
```{r}
## Updated Boxplot of Receipts by Party and Year (done)
...
#---------------------------------
# 4. Plot: Stacked Bar of Funding Sources
#---------------------------------
p_funding_composition <- ggplot(
  fund_long,
  aes(
    x = amount / 1e6,
    y = candidate,
  ...
  labs(
    title = "Fundraising Composition for Top 10 Presidential Candidates",
...
p_funding_composition

## maybe change the graph
## if kept change the coloring
...
# ---- 3. YEAR & CANDIDATE DROPDOWNS (reset, stable) ----
# Default: all traces visible (all years + all candidates)
vis_all <- rep(TRUE, n_traces)
vis_by_year <- lapply(years, function(yr) trace_year == yr)

# Candidate dropdowns: one for all, one per year
cand_menus <- list()

# All-years candidate dropdown (includes all candidates)
all_year_buttons <- list(
  list(
    method = "update",
    args = list(list(visible = vis_all), list()),
...
# Per-year candidate dropdowns (only candidates from that year)
for (yr in years) {
  cand_year <- sort(unique(fund_long_plot$candidate[fund_long_plot$year == yr]))
  buttons <- list(
    list(
      method = "update",
      args = list(list(visible = vis_by_year[[which(years == yr)]]), list()),
      label = "All candidates"
    )
  )
...
# Build year buttons: visibility only for now (layout filled after)
year_buttons <- list()
year_buttons[[1]] <- list(
  method = "update",
  args = list(
    list(visible = vis_all),
    list()  # placeholder, updated after menu is built
  ),
  label = "All years"
)
...
# Year menu base (buttons filled in below)
year_menu_base <- list(
  type = "dropdown",
  direction = "down",
  buttons = NULL,  # to be set
  x = 0.2,
  y = 1.18,
  xanchor = "center",
  yanchor = "top",
...
# Fill year menu with buttons (after initial creation)
year_menu_base$buttons <- year_buttons

# Precompute updatemenus for all-years and per-year views
updatemenus_all <- list(year_menu_base, cand_menus[["all"]])
updatemenus_by_year <- lapply(years, function(yr) list(year_menu_base, cand_menus[[as.character(yr)]]))

# Now update each year button's layout arg to swap the candidate menu
year_buttons[[1]]$args[[2]] <- list(
  updatemenus = updatemenus_all,
  xaxis = list(title = "")
)
for (i in seq_along(years)) {
  year_buttons[[i + 1]]$args[[2]] <- list(
    updatemenus = updatemenus_by_year[[i]],
    xaxis = list(title = "")
  )
}

# After updating buttons, refresh in year_menu_base
year_menu_base$buttons <- year_buttons
...
## temporary
## make better and interpretation -> Ayush
...
pca_input <- fund_long |>
  # Sum amounts for each candidate-source pair
  group_by(candidate, source) |>
  summarise(amount = sum(amount, na.rm = TRUE), .groups = "drop") |>
  
  # Convert to wide format (one row per candidate)
  pivot_wider(
    names_from = source,
    values_from = amount,
    values_fill = 0
...
# Add candidate name as label column
pca_input$label <- pca_input$candidate

# ---------------------------------------------------
# 2. Run biplot
# ---------------------------------------------------

draw_biplot(pca_input, key_axis = "label")
...
# ---------------------------------------------------
# 1. Prepare data for PCA - 2020
# ---------------------------------------------------
pca_input_2020 <- fund_long |>
  filter(year == 2020) |>
...
# Add candidate name as label column
pca_input_2020$label <- pca_input_2020$candidate

# Remove candidate column (keep only numeric columns + label)
pca_data_2020 <- pca_input_2020 |> select(-candidate)

# ---------------------------------------------------
# 2. Prepare data for PCA - 2024
# ---------------------------------------------------
pca_input_2024 <- fund_long |>
  filter(year == 2024) |>
...
# Add candidate name as label column
pca_input_2024$label <- pca_input_2024$candidate

# Remove candidate column (keep only numeric columns + label)
pca_data_2024 <- pca_input_2024 |> select(-candidate)

# ---------------------------------------------------
# 3. Create biplots
# ---------------------------------------------------
# Biplot for 2020
draw_biplot(pca_data_2020) + 
  labs(title = "PCA Biplot: Campaign Funding Sources (2020)")

# Biplot for 2024
draw_biplot(pca_data_2024) + 
  labs(title = "PCA Biplot: Campaign Funding Sources (2024)")
```
...
# make dotted and interactive -> Ayush
...
#---------------------------------
# Select top 10 spenders per year
#---------------------------------
...
#---------------------------------
# Plot with fixed x-axis visibility
#---------------------------------
p1 <- ggplot(top_spenders,
  ...
  scale_x_continuous(
    ...
    expand = expansion(mult = c(0, 0.05))   # <-- adds 5% padding to right side
  ) +
  labs(
    title = "Top 10 Presidential Campaigns by Total Spending",
    x = "Total Disbursements (Millions USD)",
    y = "Candidate",
    fill = "Party"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    ...
    legend.position = "bottom",
    plot.margin = margin(20, 20, 20, 20)     # <-- adds outer padding
  )
...
```{r}
# p2_cash <- candidates |>
#   filter(total_receipt > 100000) |>
#   mutate(
#     cash_pct = cash_on_hand_cop / pmax(total_receipt, 1) * 100
#   ) |>
#   # Reorder party by median cash_pct within each year (descending)
#   group_by(election_year) |>
#   mutate(
#     party_simple = fct_reorder(party_simple, cash_pct, .fun = median, .na_rm = TRUE, .desc = TRUE)
#   ) |>
#   ungroup() |>
#   ggplot(aes(x = cash_pct, y = party_simple, fill = party_simple)) +
#   geom_boxplot(alpha = 0.8, outlier.alpha = 0.3, outlier.size = 2) +
...
#     "Others" = "#95A3A6"
#   )) +
#   facet_wrap(~ election_year, ncol = 1, strip.position = "top", scales = "free_y") +
#   labs(
#     title = "Distribution of Campaign Cash on Hand by Party",
#     subtitle = "Parties sorted by median cash on hand (log scale)",
#     x = "Cash on Hand (%)",
#     y = NULL,
#     fill = "Party"
#   ) +
#   theme_minimal(base_size = 13) +
#   theme(
#     legend.position = "none",  # Remove legend since party names are on y-axis
